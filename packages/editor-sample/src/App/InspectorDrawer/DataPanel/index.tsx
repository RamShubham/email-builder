      ...
      {isEmpty(rows) ? (
        <div className="flex flex-col items-center gap-3 py-4 px-2 text-center">
          ...
          <div className="flex flex-col gap-1">
            <p className="text-sm font-medium text-gray-600">No variables yet</p>
            <p className="text-xs text-gray-400 leading-relaxed">
              Use the <span className="inline-flex items-center gap-0.5 px-1 py-0.5 bg-violet-50 text-violet-600 rounded text-[10px] font-medium"><Braces className="h-2.5 w-2.5" /> Variable</span> button on any text field to insert a variable, or type <code className="px-1 py-0.5 bg-gray-100 rounded text-[10px] font-mono">{'{{variableName}}'}</code> directly in your content.
            </p>
          </div>
        </div>
      ) : (
