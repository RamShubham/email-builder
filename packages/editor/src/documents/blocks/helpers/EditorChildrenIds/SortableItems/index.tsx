...
const elevatedZIndexTypes = ['Container', 'ColumnsContainer'];

const getZIndex = ({ isSelected, isDragging }: { isSelected: boolean, isDragging: boolean }) => {
...
export default function SortableItem({ id, children, isDropSuccess = false, blockId }: SortableItemProps) {
  ...
  const hasElevatedZIndex = elevatedZIndexTypes.includes(document[id]?.type || '');
  ...
  const style: React.CSSProperties = {
    ...
    position: 'relative',
    zIndex: getZIndex({ isSelected: hasElevatedZIndex, isDragging }),
    touchAction: 'none',
  };
